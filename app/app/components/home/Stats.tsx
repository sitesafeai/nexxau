...
export default function Stats() {
  ...
  return (
    <div id="stats" className="bg-gray-800 py-24 sm:py-32">
        ...
        <div className="mt-16">
          <div className="grid grid-cols-1 gap-8 sm:grid-cols-2 lg:grid-cols-3">
            {stats.map((stat, index) => (
              <div
                key={stat.name}
                className={`relative rounded-2xl bg-gray-700 p-8 shadow-lg transition-all duration-500 border border-gray-600 ${
              ...
              >
                <div className="flex items-center gap-x-4">
                  ...
                  <h3 className="text-xl font-semibold text-white">{stat.name}</h3>
                </div>
                <p className="mt-4 text-base text-gray-300">{stat.description}</p>
                <div className="mt-6">
                  <div className="flex flex-wrap gap-2">
                    {stat.sources.map((source) => (
                      <span
                        key={source}
                        className="inline-flex items-center rounded-full bg-gray-600 px-2 py-1 text-xs font-medium text-gray-300"
                      >
                        {source}
                      </span>
                    ))}
