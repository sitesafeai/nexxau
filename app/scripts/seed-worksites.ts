 ...
 * Creates:
 * - Sample worksites
 * - Workers for each site
 * - Safety violations (realistic data)
 * - Custom rules
 * - Alerts
 * 
 * Run: npx tsx scripts/seed-worksites.ts
 */
    ...
    if (existingAlerts === 0) {
      ...
      for (let i = 0; i < 5; i++) {
        ...
        alerts.push({
          ...
          message: `Violation detected at ${worksite.name}`,
          severity: severities[i % severities.length],
          status: statuses[i % statuses.length],
          type: 'safety_violation',
          createdAt: date,
          acknowledgedAt: i > 0 ? new Date(date.getTime() + 300000) : null,
