...
Consumes violation state changes from Redis Streams for alert orchestration.
"""
import json
import logging
...
class ViolationStateChange:
    """Parsed violation state change event"""
    def __init__(
        self,
        message_id: str,
        violation_id: str,
    ...
    def should_send_alert(self) -> bool:
        """Check if alert should be sent (only ACTIVE and ESCALATED states)"""
        if not self.should_alert:
            return False
        
        if self.new_state in ('ACTIVE', 'ESCALATED'):
            return True
...
class ViolationStateChangeConsumer:
    ...
    def __init__(
        ...
        consumer_group: str = "alert-orchestrator",
    ...
    def read_messages(self, count: int = 10, block_ms: int = 1000) -> List[ViolationStateChange]:
    ...
    def read_new_messages(self, count: int = 10, block_ms: int = 1000) -> List[ViolationStateChange]:
